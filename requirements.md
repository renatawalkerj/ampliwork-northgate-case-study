...
- Which hyperscaler Northgate actually runs (Azure/AWS/GCP) is unverified. Azure is the leading assumption (SAP/BTP-Azure pairing convention + "tenant" being Microsoft/Entra ID's specific term, vs. AWS "Account" or GCP "Project") — two stacking but non-conclusive signals. Still an open discovery question, and AWS/GCP are kept genuinely ready given the brief's curveball mechanic, not just listed.
