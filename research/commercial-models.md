...
The vendor comps (Vertex/Sovos/ONESOURCE) show the market ceiling for "AI reads invoices, decides tax treatment" as a **platform subscription tiered by transaction/entity volume, plus an implementation fee** — this is what a repeatable SaaS-shaped version of Northgate's tool would converge toward once sold to multiple multinationals. The contingency model (VAT IT et al.) is the alternative path if the product is positioned as a recovery service rather than software — but that model requires the vendor to own more of the recoverability determination and risk, which conflicts with "Head of Tax Risk holds the veto" (a contingency vendor with no veto-holder's blessing has an incentive to overclaim recoverability). **Recommendation for the productization pitch:** platform/subscription pricing is the more defensible model given the risk-owner constraint; contingency pricing is worth naming as the "what everyone will ask about first" alternative and explaining why it's the wrong fit here.
