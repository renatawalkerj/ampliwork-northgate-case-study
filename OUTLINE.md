...
- *EMEA regional manager (9 yrs)*, practitioner reality-check — says the VP's own diagnosis is wrong: coding isn't the bottleneck, dirty supplier master data is (same supplier, 3 tax profiles). This is the single most concrete, falsifiable claim in the whole discovery call, and it's independently corroborated by real SAP architecture research (`research/sap-domain.md`) — not just her impression. Treating it as a footnote risks losing her confidence; the entity-resolution problem it points to needs to be core scope, and Phase 1 should measure and report its actual scale back to her rather than leaving her estimate unresolved.
- *APAC analyst*, practitioner reality-check — a third root cause: much of the loss is non-compliant supplier invoices, not misclassification. Independently corroborated by real research (EU Art. 226's exhaustive mandatory-field list, `research/tax-domain.md`) — same elevated evidentiary status as the EMEA manager's claim, not just her impression. Caps how much of the $8–30M a pure tax-judgment engine can ever recover, and points to a distinct, lower-risk capability (format-compliance checking + automated supplier outreach) that a coding-focused solution wouldn't otherwise address.
